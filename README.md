...
- npm 10+
- A Chromium/WebKit/Firefox browser with Web Audio API support
...
2. Start the development server (Vite on port `4444`):

   ```bash
   npm run dev
   ```

3. Open `http://localhost:4444` and interact with the app. Audio playback will begin after a user gesture (click/tap), as required by browser audio policies.
...
- Generate waveform JSON for visualizers:

  ```bash
  npm run waveforms:build
  ```

- Scaffold a new 8-voice kit template:
...
If you're browsing the codebase for the first time, this map gives you a practical starting point:

- `src/core/audio/`: Audio runtime construction, sample/waveform caching, and offline WAV export.
- `src/features/sequencer/`: Pattern grid logic, variation chaining, and sequence interaction flows.
- `src/features/instrument/`: Per-instrument UI and state (volume, pan, pitch, filters, envelope, mute/solo).
- `src/features/master-bus/`: Global FX chain controls and parameter synchronization.
- `src/features/preset/`: Save/load/share/export flows plus serialization/compression logic.
- `src/shared/`: Reusable primitives (UI controls, hooks, knob system, utility libraries).

## Data and asset conventions

- Built-in samples live in `public/samples/<kit-id>/` and are grouped into 8 instruments per kit.
- Matching precomputed waveform files live in `public/waveforms/<kit-id>/` for lightweight rendering.
- Default kit definitions are stored in `src/core/dhkit/defaults/*.dhkit`.
- Presets can be persisted in-browser, exported as `.dh`, and shared through compressed URL payloads.
